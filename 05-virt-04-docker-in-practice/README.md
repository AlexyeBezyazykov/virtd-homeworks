...
не могу разобраться, контейнер Web перезагружается, из за того что не может подключиться в базк данных. перепробовал кучу вариантов, подглядывал в ответы на домашки других студентов. из за этого не могу продвинуться дальше.
```
